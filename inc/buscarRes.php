...
try {
    ...
    // Consulta base
    $sql = "SELECT DISTINCT tbl_restaurante.id_restaurante, tbl_restaurante.nombre_restaurante, tbl_restaurante.precio_medio, tbl_restaurante.valoracion, tbl_restaurante.imagen_res FROM tbl_restaurante INNER JOIN tbl_comida_restaurante ON tbl_restaurante.id_restaurante = tbl_comida_restaurante.id_restaurante";
    // Array para almacenar las condiciones
    $conditions = [];
    $zero = '';
    // Agregar condiciones según los filtros
    if (!empty($nombreResta)) {
        $conditions[] = "tbl_restaurante.nombre_restaurante LIKE :restaurante";
    }
